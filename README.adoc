[[spring-boot-jsp-executable-jars]]
...
[[spring-boot-jsp-sample-project]]
=== Sample Project
...
http://undertow.io/[Undertow] is another alternative for using an embedded container with Spring Boot. While I was working on updating the Spring Boot documentation regarding JSP support for Tomcat, I noticed the following line in the Reference Guide for Spring Boot `1.3.3`:
...
Being a good citizen, I dug a little deeper and discovered the https://github.com/fourcube/undertow-jsp-sample[Undertow JSP sample] application. It turns out that Undertow has indeed JSP support by using a https://github.com/undertow-io/jastow[jastow], which is a Jasper fork for Undertow.

So the key was to adapt the Undertow JSP sample application for Spring Boot.
